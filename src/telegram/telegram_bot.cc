...
bool TelegramBot::SendText(const std::string& text) noexcept {
  ...
  if (CURL *curl = curl_easy_init(); curl) {
    std::string data = "chat_id=" + chat_id_ +  "&disable_web_page_preview=1&text=" + text;
    ...
    curl_easy_setopt(curl, CURLOPT_HTTP_VERSION, static_cast<long int>(CURL_HTTP_VERSION_2TLS));
